...
$(document).ready(function() {
    if ((page == "categoryFaq") || (page == "categoryArticles")) {
        ...
        $.ajax({
                url: base_url + 'api/websitePage/getData',
                type: 'POST',
                data: { page: pagem },
            })
            .done(function(response) {
                ...
                $.each(_.uniqBy(data["sections"], pagem + "Category"), function(index, val) {
                    ...
                    $.each(order, function(index, value) {
                        if (order[index] == "Action") {
                            html += "<td><a class='editRow' href='#' value='" + val[bodyOrder[order[0]]] + "'><i class='fa fa-pencil-alt'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;<a class='deleteRow' href='#' value='" + val[bodyOrder[order[0]]] + "'><i class='fas fa-trash-alt'></i></a></td>";
                        } else {
                ...
                $(document).on('click', '.deleteRow', function() {
                    ...
                    $.ajax({
                            url: base_url + 'api/websiteManagement/deleteCategory',
                            type: 'POST',
                            data: { page: pagem, category: id },
                        })
                        .done(function(data) {
                            if (JSON.parse(data) == true) {
                                location.reload();
                ...
                $(document).on('click', '.editRow',async function() {
                    var id = $(this).attr('value');
                    const { value: renameCategory } = await Swal.fire({
                        title: 'Rename Category To',
                        input: 'text',
                        inputValue: id,
                    ...
                    if (renameCategory) {
                        $.ajax({
                                url: base_url + 'api/websiteManagement/editCategory',
                                type: 'POST',
                                data: { page: pagem,category:id,renamecategory: renameCategory},
                            })
                            .done(function(data) {
                                if (JSON.parse(data) == true) {
                                    location.reload();
    ...
    $.ajax({
        ...
        .done(function(response) {
            ...
            $.each(data['webpage'], function(index, val) {
                ...
                $.each(order, function(index, value) {
                    if (order[index] == "Action") {
                        html += "<td><a target='_blank' href='" + base_url + "page/" + val["link"] + "'>View</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href='addPage?id=" + val["id"] + "'><i class='fas fa-pencil-alt'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;<a class='deleteRow' href='#' value='" + val["id"] + "'><i class='fas fa-trash-alt'></i></a></td>";
                    } else {
                        html += "<td>" + val[bodyOrder[order[index]]] + "</td>";
                    }
                });
    ...
    $(document).ready(function() {
        if ((page == "addFaq") || (page == "addArticles")) {
            $.ajax({
                    url: base_url + 'api/websitePage/getData',
                    type: 'POST',
                    data: { page: page.slice(3, page.length).toLowerCase() },
                })
                .done(function(response) {
                    var data = JSON.parse(response);
                    $.each(_.uniqBy(data["sections"], page.slice(3, page.length).toLowerCase() + "Category"), function(index, val) {
                        var newState = new Option(val[page.slice(3, page.length).toLowerCase() + "Category"], val[page.slice(3, page.length).toLowerCase() + "Category"], false, false);
                        $("#" + page.slice(3, page.length).toLowerCase() + "Category").append(newState).trigger('change');
