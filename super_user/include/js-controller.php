...
$(document).ready(function() {
    ...
    $.ajax({
            ...
            type: 'POST',
            data: { page: page, userId: <?php echo $_SESSION["UserData"]->id; ?>}
        })
        .done(function(response) {
            ...
            if (_.isNull(data["menuRoles"])) {
                $("#customMenu").remove();
            }else{
            $.each(JSON.parse(data["menuRoles"].role), function(key, val) {
                 if (val==false) {
                    $("#customMenu li[data-liSelector"+key+"]").remove()
                 }
            });
            }
